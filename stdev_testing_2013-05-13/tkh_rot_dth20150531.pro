...
PRO TKH_Rot_DTH20150531, N_trajectories = N_trajectories, $ ;Number of output files/trajectories
...
IF N_Elements(D_ROTATION) NE 0 THEN $
     Rotation_Jumps = TKH_ROT_JUMP_DISTRIBUTION_20130513(D_ROTATION[n], N_POINTS=100000) $
   ELSE Rotation_Jumps = make_array(100000,value=rotation_angle[n])
print, '...', d_rotation[n];help, rotation_jumps
...
;Vout[*,100000*step+1] = Vout[*,100000*step];
Vout[*,0] = Vout[*,100000]
...
get_lun, lun
openW, lun, txtfile
for f = 0L, N_Steps/4L -1 do begin
printf, lun, VOut[*,f]
endfor
for f = N_Steps/4L, N_Steps/2-1L do begin
printf, lun, VOut[*,f]
endfor
for f = N_Steps/2L, (N_Steps*3/4)-1L do begin
printf, lun, VOut[*,f]
endfor
for f = N_Steps*3/4L, N_Steps-1L do begin
printf, lun, VOut[*,f]
endfor
close, lun ; closes file (need to do this or no copy will exist)
free_lun, lun 
