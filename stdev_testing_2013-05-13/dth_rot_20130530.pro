...
PRO dth_Rot_20130530, N_trajectories = N_trajectories, $ ;Number of output files/trajectories
...
V_File = dialog_pickfile(/read, filter = '*.dat', $
 title = 'If you wish to continue from previous trajectory(ies), select a V_Out file.')
...
IF V_File NE '' THEN BEGIN 
  ...
  V_last = fltarr(3,n_trajectories)
  IF V_File_Info.exists EQ 1 THEN BEGIN
     file_parts = strsplit(v_file, 'trajectory', /extract, /regex)
     
     for i=0, n_trajectories-1 do begin
     
     oldfile = strcompress(file_parts[0]+'trajectory_extddd'+string(i+1)+'.dat',/remove_all)
...
IF N_Elements(V_Init) EQ 0 THEN BEGIN 
  V_init = [0.57735027,0.57735027,0.57735027]  
  ENDIF ELSE BEGIN
  V_init = V_last
ENDELSE
...
;Vout[*,0] = V_Init;<--move this to inside the while statement so it can vary!!
...
;Set up file for the trajectory
 filename = strcompress(file + 'trajectory_extdddd'+ string(n+round(Start_Number)) + '.dat',/remove_all)
...
;-------------------------------------------------------------
;now need to concatenate this array with the previous before overwriting files!
file_full_path = strcompress(file_parts[0] + '/trajectory_extddd' + string(n+start_number) + '.dat',/remove_all)
...
OpenR, lun_2, file_full_path, /get_lun 
...
;get_lun, lun
;openW, lun, txtfile
;for f = 0, N_Steps/4 -1 do begin
;printf, lun, VOut[*,f]
;endfor
;for f = N_Steps/4, N_Steps/2-1 do begin
;printf, lun, VOut[*,f]
;endfor
;for f = N_Steps/2, (N_Steps*3/4)-1 do begin
;printf, lun, VOut[*,f]
;endfor
;for f = N_Steps*3/4, N_Steps-1 do begin
;printf, lun, VOut[*,f]
;endfor
;close, lun ; closes file (need to do this or no copy will exist)
;free_lun, lun 
