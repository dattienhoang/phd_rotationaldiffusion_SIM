...
;START_NUMBER
  ...
  ;dth - d_rotation and rotation_angles are arrays with new taus or rotation angles for a heterogeneous scenario!
  
PRO TKH_Rot_DTH20150731B, N_trajectories = N_trajectories, $ ;Number of output files/trajectories
              ...
              n_pts = n_pts, $              ;steps in a single trajectory
...
print, '...d_rotational (tkh_rot.pro) d_rotational = ', d_rotation[0]
...
IF N_Elements(n_pts) EQ 0 THEN n_pts=500000
...
IF (N_Elements(rotation_angle)+ N_Elements(D_Rotation)) EQ 0 $
     THEN begin
     rotation_angle = fltarr(n_trajectories) + 1.2*!DPI/180 ;0.034906585D
...
IF V_File NE '' THEN BEGIN 
  ...
  ENDIF ELSE GOTO, ENDPROGRAM ;of file exists
...
;*********
FOR i=1, n_elements(texs[*,0])-1 DO BEGIN
  IF texs[i-1,n] GT n_pts THEN GOTO, NOMORETEXS
  IF fix(texs[i,n]-texs[i-1,n]) LT 2 THEN BEGIN
    texs[i,n] = texs[i-1,n]
    GOTO, SKIPTEX
  ENDIF
  Vout = dblarr(3,ulong((texs[i,n]-texs[i-1,n])+1))
  Vout[*,0] = V_Init
  ;Now calculate trajectory in 100,000 point chunks for this tau_ex
  ;FOR step = 0, fix((texs[i,n]-texs[i-1,n]-1.)/100000) DO BEGIN
    ;print, texs[i,n]-texs[i-1,n]
    ...
    randnumbers = randomu(seed, 2, ulong((texs[i,n]-texs[i-1,n])), /double)
    ;Define a matrix to hold the angles for the rotation axes
    angles = dblarr(2,ulong((texs[i,n]-texs[i-1,n])))
    
    ;Define masks for the theta and phi angles
    phi_mask = findgen(2,ulong((texs[i,n]-texs[i-1,n]-1.)))/2
    ...
    IF N_Elements(D_ROTATION) NE 0 THEN $
         Rotation_Jumps = ks_rot_dist_idl(D_ROTATION[i,n], N_POINTS=ulong(texs[i,n]-texs[i-1,n])) $; TKH_ROT_JUMP_DISTRIBUTION_20130513
       ELSE Rotation_Jumps = make_array(ulong(texs[i,n]-texs[i-1,n]),value=rotation_angle[n])
       
    ;Calculate quaternion axis vectors from rotation axis angles then calculate
    ;quaternion
    Quaternionaxis = dblarr(3,ulong((texs[i,n]-texs[i-1,n])))
    Q = dblarr(4,ulong((texs[i,n]-texs[i-1,n])))
    ...
    FOR j=0L, ulong((texs[i,n]-texs[i-1,n]))-1 DO BEGIN ;100,000 iterations
      ;Calculate new orientation
      Vout[*,j+1] = QTVROT(Vout[*,j], Q[*,j])
      ;Renormalize to remove rounding errors
      IF (j+1) mod 100 EQ 0 THEN Vout[*,j+1] = Vout[*,j+1]/(total(Vout[*,j+1]^2))
    ENDFOR
    
    IF N_Elements(waittime) NE 0 THEN wait, waittime
    ;help, vout
    ;print, 'Loop #', step, ' took ', systime(1)-starttime, ' seconds.'
    WriteU, lun, float(Vout[*,0:texs[i,n]-texs[i-1,n]]);WriteU, lun, float(Vout)
    ;help, Vout[*,0:texs[i,n]-texs[i-1,n]];print, texs[i,n]-texs[i-1,n]
    ;move final orientation up to initial position for next iteration
    Vout[*,0] = Vout[*,texs[i,n]-texs[i-1,n]];Vout[*,0] = Vout[*,100000]
  ;ENDFOR ;(n_pts)
  ;print, vout
SKIPTEX:
NOMORETEXS:
...
get_lun, lun
openW, lun, txtfile
for f = 0L, n_pts/4L -1 do begin
printf, lun, VOut[*,f]
endfor
for f = n_pts/4L, n_pts/2-1L do begin
printf, lun, VOut[*,f]
endfor
for f = n_pts/2L, (n_pts*3/4)-1L do begin
printf, lun, VOut[*,f]
endfor
for f = n_pts*3/4L, n_pts-1L do begin
printf, lun, VOut[*,f]
endfor
